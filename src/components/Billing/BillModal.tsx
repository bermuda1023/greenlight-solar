  ...
  const handlePostAllBills = async () => {
    const billsToProcess = selectedCustomers
      .map((customerId) => {
        ...
        const parameter = parameters[0]; // Assuming we only have one set of parameters

        const billResult = calculateBilling({
          energyConsumed: 500,
          startDate: new Date(startDate || ""),
          endDate: new Date(endDate || ""),
          fuelRate: parameter?.fuelRate || 0.14304,  // Use fetched parameter or default
          energyExported: 150,
          basePrice: parameter?.basePrice || 0.15,   // Use fetched parameter or default
          feedInPrice: parameter?.feedInPrice || 0.5, // Use fetched parameter or default
        });
