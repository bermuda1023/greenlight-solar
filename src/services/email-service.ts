...
/**
 * Email service for sending daily status reports to administrators
 */
export class EmailService {
  ...
  /**
   * Send daily status report email to admin
   */
  async sendDailyStatusReport(
    ...
    try {
      ...
      const mailOptions = {
        from: `"Greenlight Energy - Daily Report" <${process.env.EMAIL_USER}>`,
        replyTo: process.env.EMAIL_USER,
        to: adminEmail,
        subject: `Daily API & Production Status Report - ${new Date(reportData.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`,
        html: htmlContent,
      };

      const info = await this.transporter.sendMail(mailOptions);

      console.log('[EMAIL] Daily status report sent successfully. Message ID:', info.messageId);
      ...
      console.error('[EMAIL] Error sending daily status report:', error);
      return {
        success: false,
